...
import info.pavie.basicosmparser.controller.BinExporter;
import info.pavie.basicosmparser.controller.CSVExporter;
import info.pavie.basicosmparser.controller.GpxExporter;
import info.pavie.basicosmparser.controller.JsonGarminExporter;
import info.pavie.basicosmparser.controller.OSMParser;
import info.pavie.basicosmparser.controller.TrueBinExporter;
...
import com.tgb.controller.GfxExporter2;
		...
		if(args.length != 2) {
			...
			if(!input.exists() || !input.isFile() || !input.canRead()) {
				...
				if(!output.exists() || !output.isDirectory()) {
					...
					//BinExporter exporterBin = new BinExporter();
					GfxExporter2 exporterGfx = new GfxExporter2();
					GpxExporterMultTracks exporterGpxMulti = new GpxExporterMultTracks();
					
					try {
						//exporter.export(parser.parse(input), output);
						//exporterBin.export(parser.parse(input), output);
						DrawMap3 myDrawer = new DrawMap3(exporterGpxMulti);
						
						exporterGfx.export(parser.parse(input), myDrawer);
						
						myDrawer.createAndShowMap();
						
						// Now called by the map!
						exporterGpxMulti.export(parser.parse(input), output, myDrawer);				
