...
//OTHER METHODS
	...
	public void export(Map<String,Element> elements, File outputFolder) throws IOException {
		...
		StringBuilder waysAllNextLon = new StringBuilder("var allLon = ["); // Firsts are always lat+lon. Next will be diffs if not too big
		StringBuilder waysAllNextLat = new StringBuilder("var allLat = [");
		...
		for(String id : elements.keySet()) {
			...
			if(currentElem instanceof Node) {
				...
				if (currentElem.isVisible())
				{
					...
					int apprLonPrev =new Double(lonPrev*100000).intValue();
					int apprLatPrev =new Double(latPrev*100000).intValue();
					
					waysAllNextLon.append(apprLonPrev+",");
					waysAllNextLat.append(apprLatPrev+",");
					...
					for(int i=1; i < currentWay.getNodes().size(); i++) {
						...
						int apprLonCur =new Double(lonCur*100000).intValue();
						int apprLatCur =new Double(latCur*100000).intValue();

						apprLonPrev =new Double(lonPrev*100000).intValue();
						apprLatPrev =new Double(latPrev*100000).intValue();

						int diffLon = apprLonCur - apprLonPrev;
						int diffLat = apprLatCur - apprLatPrev;

						// Stay in 2**12 ?
						if (diffLon > 4096)
						{
							System.out.println("Overload longitude "+diffLon);
						}
						if (diffLat > 4096)
						{
							System.out.println("Overload latitude "+diffLat);
						}
						
						waysAllNextLon.append(diffLon+",");
						waysAllNextLat.append(diffLat+",");
