...
//OTHER METHODS
	...
	public void export(Map<String,Element> elements, File outputFolder) throws IOException {
		...
		StringBuilder waysAllNextLon = new StringBuilder("var allLonLat = [");
		StringBuilder roadAllNextLon = new StringBuilder("var allRoadLonLat = [");
      ...
      // Firsts are always lat+lon. Next will be diffs if not too big
		...
		for(String id : elements.keySet()) {
			...
			if(currentElem instanceof Node) {
				...
				if (currentElem.isVisible())
				{
					...
					boolean isPathway = false; // Remove all unknown polygons
					...
					if (isRoad || isPathway)
					{
						...
						for(int i=1; i < nbOfNodes; i++) {
							...
							if (isRoad)
							{
								roadAllNextLon.append(newLon+", "+prevLon+", ");
							}
							else
							{
							...
							if (i % 50 == 0)
							{
								System.out.println("IGNORED - Cutting at "+i+" elements");
		...
		roadAllNextLon.deleteCharAt(roadAllNextLon.length()-1);
